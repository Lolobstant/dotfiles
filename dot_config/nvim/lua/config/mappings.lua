...
if is_available('telescope.nvim') then
  ...
  if is_available('telescope-file-browser.nvim') then
    M.n['<leader>e'] = { function() extensions.file_browser.file_browser({ path = '%:p:h', select_buffer = true }) end,
  ...
  if is_available('neovim-project') then
    ...
    M.n['<leader>Wl'] = { function() extensions["neovim-project"].discover() end, '[W]orkspace [L]ist' }
    M.n['<leader>Wr'] = { function() extensions["neovim-project"].history() end, '[W]orkspace [R]ecent' }
  ...
  if is_available('nvim-notify') then
    ...
    M.n['<leader>sn'] = { function() require('telescope').extensions.notify.notify() end, '[S]earch [N]otifications' }
...
if is_available('mini.files') then
  ...
  M.n['<leader>e'] = { function() mini_files.open() end,
    { desc = 'Toggle [E]xplorer', noremap = true } }
  M.n['<leader>E'] = {
    function()
  ...
  -- TODO: deplacer dans un fichier autocmd
  ...
  vim.api.nvim_create_autocmd('User', {
    group = vim.api.nvim_create_augroup('set_mini_files_directory', {}),
...
if is_available("mini.bufremove") then
  M.n["<leader>bd"] = { function() require("mini.bufremove").delete(0, false) end, "[B]uffer [D]elete" }
  M.n["<leader>bD"] = { function() require("mini.bufremove").delete(0, true) end, "[B]uffer [D]elete (force)" }
else
...
M.n["<leader>bf"] = { function() vim.lsp.buf.format({ async = true }) end, "[B]uffer [F]ormat" }
...
if is_available('toggleterm.nvim') then
  ...
  vim.keymap.set({ 'n', 't' }, "<leader>th", "<cmd>ToggleTerm size=10 direction=horizontal<cr>",
    { desc = "[T]oggleTerm [H]orizontal split" })
  vim.keymap.set({ 'n', 't' }, "<leader>tv", "<cmd>ToggleTerm size=80 direction=vertical<cr>",
...
if vim.g.neovide then
  ...
  M.n["<S-UP>"] = { function() change_scale_factor(1.05) end, "Scale UP" };
  M.n["<S-DOWN>"] = { function() change_scale_factor(1 / 1.05) end, "Scale DOWN" }
end
...
M.n['<leader>dm'] = { function() vim.diagnostic.open_float({ border = "rounded" }) end, '[D]iagnostics [M]essage' }
...
-- NOTE: remove lazy-nvim default keymaps
-- vim.keymap.del("n", "<leader>l")
-- TODO: move to autocmd file
